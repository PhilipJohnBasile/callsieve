...
- Added an Agent-Memory-Protocol-aligned MCP memory surface: `callsieve_memory_recall` (read-only `amp.recall`), `callsieve_memory_stats` (`amp.stats`), `callsieve_memory_export` (`amp.export`), `callsieve_memory_import` (`amp.import`), and `callsieve_memory_forget` (`amp.forget`). Memory-verb failures return stable error codes (`AMP_INVALID_ARGUMENT`, `AMP_INTERNAL`) instead of opaque strings.
