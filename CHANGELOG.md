...
- Added `--embed-model <small|code>` to `index` and `agent-context` (embed builds): `code` selects jina-embeddings-v2-base-code, a code-tuned 768-dim model trained on docstring-to-code pairs, as an opt-in quality tier. The default stays BGE-small. The embeds cache is keyed by model id, so switching models rebuilds it automatically. Benchmarks select the model via `CALLSIEVE_BENCH_EMBED_MODEL=code`.
