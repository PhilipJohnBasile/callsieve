...
1. **Graph-consensus recall**: natural-language queries get candidates boosted when multiple top-ranked files agree via import/reference edges — formally validated on the public benchmarks (NL slice 20.0% → 33.3% lexical / 36.7% hybrid; the 50-issue identifier set holds 60.0% with zero losses; the adjacency study showed 82% of misses were one hop from the pool). Deterministic and explainable: the packet says which top candidates agree.
