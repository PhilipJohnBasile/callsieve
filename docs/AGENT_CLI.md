...
- array order: ordering signal. Default skim omits per-file `rank`, `score`, and `selection_confidence`; normal/full packets may use `score` plus `selection_confidence`.
...
- `i`: compact skim impact array. Positions are `[risk, tests, upstream, downstream, flags]`; `tests` is a string for one external test path, a number for one zero-based `context.read_first` index, or an array for multiple test paths and read-first indexes. Trailing zero counts are omitted. Skim risk codes are `l`, `m`, and `h` for low, medium, and high. Optional `flags` names which compact graph evidence exists: `test`, `im`, `call`, `ref`, and `by`. Normal/full packets may use `impact`.
...
callsieve agent-native-protocol --out benchmarks/public/results/agent-native-protocol.json
callsieve agent-native-template benchmarks/public/repos/psf/requests benchmarks/public/manifest.json --k 5 --out benchmarks/public/results/agent-native-requests-template.json
callsieve agent-native-check benchmarks/public/results/agent-native-requests-template.json --mode measured --source-artifact <transcript-or-export> --out benchmarks/public/results/<tool>-check.json
