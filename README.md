...
CallSieve is now an open-source local Rust CLI with a JSON index, deterministic retrieval, optional local embeddings, optional LSP reference enrichment, CODEOWNERS and git-history signals, stack-trace-aware error context, lifecycle hooks for Codex, Claude Code, GitHub Copilot, OpenCode, Antigravity CLI, and Cline, MCP/rule/template setup for Cursor, VS Code, Windsurf, Continue, Zed, Junie, JetBrains AI Assistant, Amp, Goose, Warp, and Zoo Code, context-first guardrails, a daemon that keeps the index fresh and serves agent-context from memory over a local socket (0.31s vs 0.61s direct on a 2.7k-file repo, byte-identical output), team index warm starts via `index-export`/`index-import`, one-command `setup-auto` agent detection, benchmark reports, observed-session traces, and gated proof reports.

Public retrieval result (June 2026): lexical first-correct-file@5 is `60.0%` on the 50-issue SWE-bench Lite subset in `benchmarks/public/manifest-50.json`, and opt-in hybrid retrieval is strictly non-regressing — identifier-kind queries keep lexical order, so hybrid also scores `60.0%` there (50 ties, zero losses), while the 30-issue natural-language slice in `benchmarks/public/manifest-nl.json` gains from semantic reranking: `23.3%` with the default BGE-small model and `26.7%` with the opt-in code-tuned model (`--embed-model code`), vs `20.0%` lexical. Recall gains come from reranking the lexical candidate pool; both slices remain well above naive grep. A measured next step exists: 82% of the remaining natural-language misses are one import/reference/directory hop from files lexical already retrieves (`benchmarks/public/results/nl-miss-graph-adjacency-study.json`).
