...
Public retrieval result (June 2026, v0.3.2): on the 50-issue SWE-bench Lite subset in `benchmarks/public/manifest-50.json`, rarity-scaled filename-stem ranking lifts lexical first-correct-file@5 from `56.0%` to `60.0%` (+4.0 pp, 2 fixes, 0 regressions). On the 30-issue natural-language slice in `benchmarks/public/manifest-nl.json`, the default BGE-small hybrid scores `23.3%` vs `20.0%` lexical (+3.3 pp), and the opt-in code-tuned model (`--embed-model code`, jina-embeddings-v2-base-code) reaches `26.7%` (+6.7 pp, 2 wins, 0 losses) — double the BGE-small lift, through better reranking. Union-pass injection still does not fire; recall gains so far come from reranking the lexical candidate pool. Both slices remain well above naive grep.
