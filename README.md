...
Public hybrid A/B result (June 2026, chunk-cap-8 release): lexical and opt-in hybrid retrieval both scored `56.0%` first-correct-file@5 (`+0.0 pp`, 50 ties) on the 50-issue SWE-bench Lite subset in `benchmarks/public/manifest-50.json`. The 30-issue natural-language slice in `benchmarks/public/manifest-nl.json` improved from `20.0%` lexical to `23.3%` hybrid (`+3.3 pp`, 1 win, 0 losses, 29 ties) — semantic reranking within the existing candidate pool lifted one vocabulary-gap issue without requiring union-pass injection. BGE-small cosines for full vocabulary-gap misses stay below 0.10 even with 8 indexed symbols per file; a stronger embedding model is the next lever. Both slices remain well above naive grep: `+50.0 pp` on the 50-issue set and `+9.7 pp` on the natural-language slice.
