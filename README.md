...
Public retrieval result (June 2026, graph-consensus validation): lexical first-correct-file@5 is `60.0%` on the 50-issue SWE-bench Lite subset in `benchmarks/public/manifest-50.json` (10× the naive-grep baseline of `6.0%`), and opt-in hybrid retrieval is strictly non-regressing — `60.0%` there too, 50 ties, zero losses. On the 30-issue natural-language slice in `benchmarks/public/manifest-nl.json`, graph-consensus ranking (files endorsed by multiple top candidates via import/reference edges, with the endorsement-ceiling guard) lifts the default deterministic path from `20.0%` to `33.3%` with no embeddings, and hybrid reranking on top reaches `36.7%` (grep baseline: `13.3%`). The mechanism came from a checked-in measurement: 82% of natural-language misses were one graph hop from files lexical already retrieved (`benchmarks/public/results/nl-miss-graph-adjacency-study.json`).
