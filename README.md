...
Public hybrid A/B result (June 2026, query-kind cosine floor release): lexical and opt-in hybrid retrieval both scored `56.0%` first-correct-file@5 (`+0.0 pp`, 0 wins, 0 losses, 50 ties) on the 50-issue SWE-bench Lite subset in `benchmarks/public/manifest-50.json`. The 30-issue natural-language slice in `benchmarks/public/manifest-nl.json` also stayed flat at `20.0%` lexical and `20.0%` hybrid (`+0.0 pp`, 1 win, 1 loss, 28 ties). Root-cause analysis confirms the quality ceiling is BGE-small embedding coverage, not the cosine floor: the semantic union pass now uses query-kind-aware floors (NaturalLanguage → 0.15, Identifier → 0.25) but cosine scores for vocabulary-gap misses stay below 0.15, so injection does not yet fire. Both slices remain well above naive grep: `+50.0 pp` on the 50-issue set and `+6.7 pp` on the natural-language slice.
